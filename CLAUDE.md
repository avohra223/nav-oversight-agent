...
- [x] **Phase 4 — UI.** Streamlit app rendering pre-baked AgentRun JSON
  records. Five pages: Dashboard (per-fund overview), Defect Detail
  (verdict cards + evidence chain + tool-call timeline + raw audit),
  Run Explorer (filterable historical runs with CSV export),
  Configuration (form-based policy editor over `config/policies.yaml`),
  Run Live (gated, the only page that consumes API credits). 11
  fixtures shipped at `audit/agent_runs/fixture_*.json` so the demo
  works end-to-end with no API spend. Enterprise styling: muted
  slate/stone palette, system-font stack (no CDN), no emoji.
...
- **No API spend in normal use.** Every page except Run Live reads
  pre-baked JSON from `audit/agent_runs/`. The fixtures shipped in the
  repo were generated by `scripts/generate_fixture_runs.py`, which
  invokes the actual Phase 2 tool layer to produce realistic tool
  outputs and wraps them in synthetic LLM narration.
- **Run Live gating.** Two independent gates: `ANTHROPIC_API_KEY` must
  be set in the environment AND the user must check an explicit
  acknowledgement of cost. The "Run agent now" button is disabled
  otherwise.
- **Fixture replacement.** When real runs are recorded
  (via `scripts/record_demo_runs.py`), they land in the same directory
  with the same schema and the UI displays them identically. To go back
  to a clean fixture-only state: `scripts/reset_to_fixtures.py --apply`.
- **Offline capability.** No CDN font fetches; system font stack
  (`-apple-system`, `Segoe UI`, `Helvetica Neue`, `Consolas`,
  `JetBrains Mono`). The app runs without an internet connection beyond
  what Streamlit itself requires for its own static assets at boot.
- **Styling philosophy.** No emoji. No "AI" branding. The product is the
  "NAV Defect Detection System." Severity is communicated via colour and
  text, never iconography. Numeric formatting: thousand separators on
  amounts, ISO timestamps, monospace for IDs.
